...
def search_international(country_code: str, keyword: str = None,
    ...
    conditions = ["p.country_code = @country_code", "title.language = 'en'"]
    ...
    if grant_after:
        conditions.append("p.grant_date > @grant_after")
        params.append(ScalarQueryParameter("grant_after", "INT64", grant_after))

    assignee_join = ""
    if assignee:
        assignee_join = ", UNNEST(p.assignee_harmonized) AS a"
    ...
    WHERE
      {where}
    GROUP BY p.publication_number, p.country_code, p.kind_code
    ORDER BY grant_date DESC
    LIMIT {limit}
    """
    client = get_client()
...
def get_patent_detail(publication_number: str, force: bool = False) -> list:
    ...
    # Use scalar subqueries for title/abstract to avoid GROUP BY on ARRAY fields
    sql = f"""
    SELECT
      p.publication_number,
      p.country_code,
      ...
      p.assignee,
      p.pct_number,
      (SELECT t.text FROM UNNEST(p.title_localized) AS t WHERE t.language = 'en' LIMIT 1) AS title,
      (SELECT SUBSTR(a.text, 1, 500) FROM UNNEST(p.abstract_localized) AS a WHERE a.language = 'en' LIMIT 1) AS abstract_text
    FROM {PUB} p
